...
void SetDate(std::string date,int lineNumber,std::map<int,Project*>projects,std::map<int,User*>users,std::map<int,Task*>tasks,std::map<int,Issue*>issues){
    ...
    std::map<int, Task*>::iterator taskIndex;
    for (taskIndex = tasks.begin(); taskIndex != tasks.end(); taskIndex++)
    {
        Task *currentTask=taskIndex->second;
        if(currentTask->getStatus()==TaskStatus::initialized){
            if(compareDate(currentTask->getStartTime(),date)==-1){
                currentTask->setStatus(TaskStatus::running);
            }
        }
        if(currentTask->getStatus()==TaskStatus::running){
            if(compareDate(currentTask->getEndTime(),date)==-1){
                currentTask->setStatus(TaskStatus::late);
            }
        }
    }
    std::map<int, Project*>::iterator projectIndex;
    for (projectIndex = projects.begin(); projectIndex != projects.end(); projectIndex++)
    {
        Project *currentProject=projectIndex->second;
        if(currentProject->getStatus()==ProjectStatus::starting){
            if(compareDate(currentProject->getStartTime(),date)==-1){
                currentProject->setStatus(ProjectStatus::running);
            }
        }else if(currentProject->getStatus()==ProjectStatus::running){
            std::vector<int> listTask=currentProject->getListTasks();
            for(int i=0;i<listTask.size();i++){
                if(tasks[listTask[i]]->getStatus()==TaskStatus::late){
                    currentProject->setStatus(ProjectStatus::late);
                }
            }
        }
        if(compareDate(currentProject->getEndTime(),date)==-1){
            currentProject->setStatus(ProjectStatus::delayed);
        }
    }
    globalDate=date;
};
...
void CreateUser(std::vector<std::string>information,std::map<int,Project*>&projects,std::map<int,User*>&users,std::map<int,Task*>&tasks,std::map<int,Issue*>&issues,int lineNumber){
    ...
    for(int i=2;i<information.size();i+=2){
        if(information[i]=="--by"){
            //create user --fullname Leader1 --role member
...
void EditUser(std::vector<std::string>information,std::map<int,Project*>&projects,std::map<int,User*>&users,std::map<int,Task*>&tasks,std::map<int,Issue*>&issues,int lineNumber){
    ...
    // std::ofstream output("UNITTESTOUTPUT.txt");
    if(information[2]!="--by"){
        std::cout<<"Error: Unexpected error in line "<<lineNumber<<std::endl;
        exit(0); 
    }else{
...
void CreateTask(std::vector<std::string>information,std::map<int,Project*>&projects,std::map<int,User*>&users,std::map<int,Task*>&tasks,std::map<int,Issue*>&issues,int lineNumber){
    ...
    for(int i=4;i<information.size();i+=2){
         if(information[i]=="--project_id"){
            ...
            Project *taskProject=projects[task->getProjectId()];
            if(compareDate(information[i+1],taskProject->getStartTime())==-1){
                std::cout<<"Error: Unexpected error in line "<<lineNumber<<std::endl;
                delete task;
                exit(0);
            }
            ...
            Project *taskProject=projects[task->getProjectId()];
            if(compareDate(information[i+1],taskProject->getEndTime())==1){
                std::cout<<"Error: Unexpected error in line "<<lineNumber<<std::endl;
                delete task;
                exit(0);
            }
            ...
            for(int i=0;i<memberIds.size();i++){//kiem tra thuoc project
                bool flag=false;
    ...
    if(projects[projectId]->getStatus()==ProjectStatus::starting){
        projects[projectId]->setStatus(ProjectStatus::running);
    }
    task->setID(taskId);
    tasks[taskId]=task;
...
void EditTask(std::vector<std::string>information,std::map<int,Project*>&projects,std::map<int,User*>&users,std::map<int,Task*>&tasks,std::map<int,Issue*>&issues,int lineNumber){
    ...
    for(int i=6;i<information.size();i+=2){
        if(information[i]=="--name"){
            ...
            if(compareDate(information[i+1],project->getStartTime())==-1){
                std::cout<<"Error: Unexpected error in line "<<lineNumber<<std::endl;
                exit(0);
            }
            ...
            if(compareDate(information[i+1],project->getEndTime())==1){
                std::cout<<"Error: Unexpected error in line "<<lineNumber<<std::endl;
                exit(0);
            }
            if(compareDate(information[i+1],editTask->getEndTime())==1){
                editTask->setStatus(TaskStatus::extended);
            }
            editTask->setEndTime(information[i+1]);
        }else if(information[i]=="--status"){
            if(taskStatus.count(information[i+1])){
                if(information[i+1]=="done"){
                    if(!editTask->isIssueResolved(issues)){
                        std::cout<<"Error: Unexpected error in line "<<lineNumber<<std::endl;
                        exit(0);
                    }
                }
    ...
    for(int i=6;i<information.size();i+=2){
        if(information[i]=="--name"){
            ...
            if(projectStatus.count(information[i+1])){
                if(information[i+1]=="finished"){
                    if(!project->isTaskDone(tasks)){
                        std::cout<<"Error: Unexpected error in line "<<lineNumber<<std::endl;
                        exit(0);
                    }
                }
